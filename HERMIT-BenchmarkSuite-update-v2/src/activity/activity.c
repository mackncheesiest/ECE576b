...
void compute_statistics_accel(struct activity_values *vals, double *hr) {
  printf("Computing statistics on the HLS-based accelerator\n");
  clock_gettime(CLOCK_MONOTONIC_RAW, &accel_TX_start);
  unsigned int i = 0;

  printf("Memcpy-ing input to the accelerator\n");
  // Memcpy the input struct to the input struct registers
  memcpy((unsigned int*) (accel_control_base_addr + OFFSET_TO_STRUCT_INPUT), vals, sizeof(struct activity_values));

  // Memcpy the HR array to the HR memory
  memcpy((unsigned int*) (accel_control_base_addr + OFFSET_TO_HR_MEMORY), hr, DEFLEN * sizeof(double));

  clock_gettime(CLOCK_MONOTONIC_RAW, &accel_compute_start);

  printf("Triggering accelerator to start execution\n");
  ...
  printf("Copying output from the accelerator back to the host\n");
...
void compute_statistics(struct activity_values *vals, double *hr) {
  printf("Computing statistics on the CPU\n");
...
int main(int argc, char *argv[]) {
  ...
  while (fgets(buf, 80, in_file)) {
    ...
    if (++(vals.i) >= vals.len) {
      ...
      printf("Collected a batch of %d samples, calling the kernel to compute statistics about them\n", DEFLEN);
